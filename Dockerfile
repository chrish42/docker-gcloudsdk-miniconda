...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y bzip2 curl unzip && rm -rf /var/lib/apt/lists/*
...
RUN curl -o /tmp/appengine.zip https://storage.googleapis.com/appengine-sdks/featured/google_appengine_1.9.15.zip && unzip /tmp/appengine.zip -d /opt && rm /tmp/appengine.zip
...
ENV PYTHONPATH /opt/google_appengine
